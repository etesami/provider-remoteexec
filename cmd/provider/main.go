...
import (
	...
	"github.com/crossplane/provider-remoteexec/apis"
	"github.com/crossplane/provider-remoteexec/apis/v1alpha1"
	remoteexec "github.com/crossplane/provider-remoteexec/internal/controller"
...
func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "RemoteExec support for Crossplane.").DefaultEnvars()
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-remoteexec",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add RemoteExec APIs to scheme")
	...
	kingpin.FatalIfError(remoteexec.Setup(mgr, o), "Cannot setup RemoteExec controllers")
